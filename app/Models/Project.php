...
class                                                                                                               Project extends Model
{
    ...
    public function pendingTasks($returnType="all"){
        if($returnType=="count"){
            return Task::where("status","pending")->where("project_id",$this->id)->count();
        }
        
        return Task::where("status","pending")->where("project_id",$this->id)->get();
    ...
    public function pendingTickets($returnType="all"){
        if($returnType=="count"){
            return Ticket::where("status","pending")->where("project_id",$this->id)->count();
        }
        
        return Ticket::where("status","pending")->where("project_id",$this->id)->get();
